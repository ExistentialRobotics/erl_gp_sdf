...
namespace erl::sdf_mapping {
    ...
    GpSdfMapping<Dtype, Dim, SurfaceMapping>::UpdateClusterQueue() {
        ...
        for (const auto& cluster_key: m_affected_clusters_) {
            ...
            if (auto itr = m_cluster_queue_keys_.find(cluster_key);
                itr == m_cluster_queue_keys_.end()) {  // new cluster
    ...
    GpSdfMapping<Dtype, Dim, SurfaceMapping>::TrainGpThread(
        ...
        const Dtype cluster_size = m_surface_mapping_->GetClusterSize();
        const Dtype area_half_size =
            cluster_size * static_cast<Dtype>(m_setting_->gp_sdf_area_scale) * 0.5f;
        ...
        while (m_candidate_gps_.empty()) {  // search until the intersection is empty
            if (area.IsValid()) {           // valid area: min < max
                ...
                m_surface_mapping_->IterateClustersInAabb(
                    area,
                    [&](const Key& cluster_key) {  // search for clusters in the area
...
#pragma omp parallel for default(none) shared(no_gps_indices)
        for (const std::size_t& i: no_gps_indices) {
            ...
            while (gps.empty()) {  // no gp found, maybe the test position is on the query boundary
    ...
    GpSdfMapping<Dtype, Dim, SurfaceMapping>::TestGpThread(
        ...
        Variances variances(Variances::RowsAtCompileTime, num_neighbor_gps);
        // cov (gx, d), (gy, d), (gz, d), (gy, gx), (gz, gx), (gz, gy)
        Covariances covariances(Covariances::RowsAtCompileTime, num_neighbor_gps);
        ...
        for (uint32_t i = start_idx; i < end_idx; ++i) {
            ...
            for (const std::size_t& j: gp_indices) {    // call selected GPs for inference
            ...
            if (flip) {  // flip the sign of the distance and gradient
