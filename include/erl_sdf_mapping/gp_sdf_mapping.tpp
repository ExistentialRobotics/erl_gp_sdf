...
bool
GpSdfMapping<Dim, Dtype, SurfaceMapping>::TestBuffer::ConnectBuffers(
...
template<int Dim, typename Dtype, typename SurfaceMapping>
GpSdfMapping<Dim, Dtype, SurfaceMapping>::GpSdfMapping(std::shared_ptr<Setting> setting, std::shared_ptr<SurfaceMapping> surface_mapping)
...
template<int Dim, typename Dtype, typename SurfaceMapping>
bool
GpSdfMapping<Dim, Dtype, SurfaceMapping>::Update(
...
template<int Dim, typename Dtype, typename SurfaceMapping>
bool
GpSdfMapping<Dim, Dtype, SurfaceMapping>::UpdateGpSdf(double time_budget_us) {
    ERL_BLOCK_TIMER();
    ERL_TRACY_FRAME_MARK_START();
...
template<int Dim, typename Dtype, typename SurfaceMapping>
void
GpSdfMapping<Dim, Dtype, SurfaceMapping>::TrainGps() {
...
template<int Dim, typename Dtype, typename SurfaceMapping>
void
GpSdfMapping<Dim, Dtype, SurfaceMapping>::TrainGpThread(const uint32_t thread_idx, const std::size_t start_idx, const std::size_t end_idx) {
...
template<int Dim, typename Dtype, typename SurfaceMapping>
[[nodiscard]] bool
GpSdfMapping<Dim, Dtype, SurfaceMapping>::Test(
    const Eigen::Ref<const Positions>& positions_in,
    Distances& distances_out,
    Gradients& gradients_out,
    Variances& variances_out,
...
template<int Dim, typename Dtype, typename SurfaceMapping>
void
GpSdfMapping<Dim, Dtype, SurfaceMapping>::SearchCandidateGps(const Eigen::Ref<const Positions>& positions_in) {
    ERL_BLOCK_TIMER();
...
template<int Dim, typename Dtype, typename SurfaceMapping>
void
GpSdfMapping<Dim, Dtype, SurfaceMapping>::SearchGpThread(const uint32_t thread_idx, const std::size_t start_idx, const std::size_t end_idx) {
    ERL_TRACY_SET_THREAD_NAME(fmt::format("{}:{}", __PRETTY_FUNCTION__, thread_idx).c_str());
    (void) thread_idx;
...
template<int Dim, typename Dtype, typename SurfaceMapping>
void
GpSdfMapping<Dim, Dtype, SurfaceMapping>::TestGpThread(const uint32_t thread_idx, const std::size_t start_idx, const std::size_t end_idx) {
    ERL_TRACY_SET_THREAD_NAME(fmt::format("{}:{}", __PRETTY_FUNCTION__, thread_idx).c_str());
    (void) thread_idx;
...
GpSdfMapping<Dim, Dtype, SurfaceMapping>::ComputeWeightedSum(
    const uint32_t i,
