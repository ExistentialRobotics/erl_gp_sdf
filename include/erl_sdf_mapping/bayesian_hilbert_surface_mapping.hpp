...
namespace erl::sdf_mapping {
    ...
    struct LocalBayesianHilbertMapSetting : common::Yamlable<LocalBayesianHilbertMapSetting<Dtype>> {
        using Covariance = covariance::Covariance<Dtype>;
        using KernelSetting = typename Covariance::Setting;

        std::shared_ptr<geometry::BayesianHilbertMapSetting> bhm = std::make_shared<geometry::BayesianHilbertMapSetting>();
        ...
        long hit_buffer_size = -1;                 // size of the hit buffer, -1 means no limit, 0 means no hit buffer
        ...
        Dtype surface_occ_prob_target = 0.99f;     // target occupancy probability for the surface points
    ...
    class BayesianHilbertSurfaceMapping : public AbstractSurfaceMapping {
        ...
        inline static const std::string kClassName = type_name<BayesianHilbertSurfaceMapping>();
        inline static const std::string kFileHeader = fmt::format("# {}", kClassName);
        inline static auto kFileFooter = "end_of_BayesianHilbertSurfaceMapping";
    ...
    public:
        ...
        using Tree = std::conditional_t<Dim == 2, geometry::OccupancyQuadtree<Dtype>, geometry::OccupancyOctree<Dtype>>;
        using TreeNode = std::conditional_t<Dim == 2, geometry::OccupancyQuadtreeNode, geometry::OccupancyOctreeNode>;
        ...
        struct LocalBayesianHilbertMap {
            ...
            std::shared_ptr<Setting> setting = nullptr;  // settings for the local Bayesian Hilbert map
            ...
            std::vector<int> surface_indices{};          // indices of the surface points being tracked
            ...
            bool
            Update(const Eigen::Ref<const Position> &sensor_origin, const Eigen::Ref<const Positions> &points);

        private:
            bool
            UpdateBhm(const Eigen::Ref<const Position> &sensor_origin, const Eigen::Ref<const Positions> &points);
        ...
        struct Setting : common::Yamlable<Setting> {
            std::shared_ptr<typename LocalBayesianHilbertMap::Setting> local_bhm = std::make_shared<typename LocalBayesianHilbertMap::Setting>();
            std::shared_ptr<TreeSetting> tree = std::make_shared<TreeSetting>();
            int hinged_grid_size = 11;
            uint32_t bhm_depth = 14;       // tree depth for the local Bayesian Hilbert map
            Dtype bhm_overlap = 0.2f;      // overlap between the Bayesian Hilbert maps
            Dtype bhm_test_margin = 0.1f;  // bhm_cluster_size * 0.5 + bhm_test_margin is the half-size of the local test region
            int test_knn = 1;              // number of nearest neighboring local Bayesian Hilbert maps to use for one test point
            int test_batch_size = 128;     // number of test points to process in one batch
            bool build_bhm_on_hit = true;  // if true, build the Bayesian Hilbert map on hit, otherwise build it with on node occupied
    ...
    private:
        ...
        std::vector<std::pair<Key, Position>> m_key_bhm_positions_{};
    ...
    public:
        ...
        bool
        Update(const Eigen::Ref<const Position> &sensor_origin, const Eigen::Ref<const Positions> &points, bool parallel);
        ...
        void
        PredictGradient(const Eigen::Ref<const Positions> &points, bool faster, bool with_sigmoid, bool parallel, Gradients &gradient) const;
...
template<>
struct YAML::convert<erl::sdf_mapping::LocalBayesianHilbertMapSettingF> : erl::sdf_mapping::LocalBayesianHilbertMapSettingF::YamlConvertImpl {};

template<>
struct YAML::convert<erl::sdf_mapping::LocalBayesianHilbertMapSettingD> : erl::sdf_mapping::LocalBayesianHilbertMapSettingD::YamlConvertImpl {};
