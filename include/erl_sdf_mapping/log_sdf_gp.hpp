...
namespace erl::sdf_mapping {
    ...
    public:
        ...
        Reset(long max_num_samples, long x_dim) override {
            ...
            auto kernel_setting = std::make_shared<covariance::Covariance::Setting>(*m_setting_->kernel);
            kernel_setting->scale = std::sqrt(3.) / m_setting_->log_lambda;
            kernel_setting->x_dim = x_dim;
    ...
    protected:
        ...
        AllocateMemory2(long max_num_samples, long x_dim) {
            std::pair<long, long> size = covariance::Covariance::GetMinimumKtrainSize(max_num_samples, 0, x_dim);
            if (m_mat_log_k_train_.rows() < size.first || m_mat_log_k_train_.cols() < size.second) { m_mat_log_k_train_.resize(size.first, size.second); }
            if (m_mat_log_l_.rows() < size.first || m_mat_log_l_.cols() < size.second) { m_mat_log_l_.resize(size.first, size.second); }
...
namespace YAML {
    template<>
    struct convert<erl::sdf_mapping::LogSdfGaussianProcess::Setting> {
        inline static Node
