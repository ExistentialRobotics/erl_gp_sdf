...
in [include/erl_gp_sdf/bayesian_hilbert_surface_mapping.hpp#L82-L211](../include/erl_gp_sdf/bayesian_hilbert_surface_mapping.hpp#L82-L211).
...
- [3D Depth Camera, Cow And Lady](../config/cow_and_lady/bayesian_hilbert/surf_mapping_float.yaml)
- [3D Lidar, Newer College](../config/newer_college/bayesian_hilbert/surf_mapping_float_scale0.1.yaml)
- [2D Lidar, Gazebo Room](../config/gazebo/bayesian_hilbert/surf_mapping_float.yaml)
...
update_map: # parameters for updating the map
    ...
filter_points: false  # whether to filter input points by their world-frame z before updating
filter_points_min_z_world: -10000.0  # min world-frame z to keep, used when `filter_points` is true
filter_points_max_z_world: 10000.0   # max world-frame z to keep, used when `filter_points` is true
...
hinged_grid_size: 11  # size of the hinged grid
bhm_overlap: 0.3   # overlap size between neighboring BHMs (in meters), used when `weight_sync` is false.
bhm_overlap_sync: 1  # overlap size between neighboring BHMs (in grid), used when `weight_sync` is true.
build_bhm_on_hit: true  # whether to build local BHM as long as there is at least one hit point
bhm_test_margin: 0.1  # extra half-size margin added to the local BHM test region
...
| `filter_points` | If true, filter input points by their world-frame z coordinate before updating the map. |
| `filter_points_min_z_world` | Minimum world-frame z of the points to keep. Only valid when `filter_points` is true. |
| `filter_points_max_z_world` | Maximum world-frame z of the points to keep. Only valid when `filter_points` is true. |
...
| `bhm_overlap` | Overlap size between neighboring BHMs (in meters), used when `weight_sync` is false. The local BHM half-size becomes $\text{bhm_node_size} / 2 + \text{bhm_overlap}$. |
| `bhm_overlap_sync` | Overlap size between neighboring BHMs (in grid cells), used when `weight_sync` is true. It defines how many hinged-grid cells are shared between neighboring BHMs. |
...
local_bhm:
    ...
    surface_grid_size: 5
    surface_log_odds: 5.0
    surface_log_odds_lr: 0.2
    surface_log_odds_num_points: 32
    surface_log_odds_min: -5.0
    surface_log_odds_max: 40.0
...
| `max_dataset_size` | Maximum number of dataset points (hit + free) to store. `-1` (or `<= 0`) means no limit. |
| `hit_point_buffer_size` | Size of the buffer to store hit points. Points in the buffer are used to estimate the surface log-odds. `-1` (or `<= 0`) means no limit. |
| `ray_buffer_size` | Size of the buffer to store rays for free space sampling. `-1` (or `<= 0`) means no limit. |
...
| `surface_log_odds_lr` | Learning rate for the exponential moving average (EMA) that estimates the surface log-odds. Given the mean log-odds $\hat{l}_t$ of newly sampled hit points, $l_t \leftarrow t\,\hat{l}_t + (1-t)\,l_{t-1}$ with $t = \text{surface_log_odds_lr}$. On the first valid estimate (when the level is still the $\pm\infty$ sentinel), it is seeded directly from $\hat{l}_t$. |
| `surface_log_odds_num_points` | Maximum number of hit points sampled from the buffer to estimate the surface log-odds $\hat{l}_t$ in each update. |
...
update_map:
    method: 2
    surface_max_abs_logodd: 2      # used when method is 1
    surface_bad_abs_logodd: 200    # used when method is 1
    surface_step_size: 0.001       # used when method is 1
    max_num_points: 50000          # used when method is 1
    max_adjust_tries: 1            # used when method is 1
    include_neighbor_bhm: true
    boundary_margin: 0.0           # used when method is 2
    max_update_dist: -1            # max sensor distance for updating BHMs, -1 means no limit
    max_num_bhm: 1500              # used when method is 2
    max_num_voxels: 8000           # used when method is 2
    var_scale: 1000000
    var_max: 0.2
```
...
| `boundary_margin` | Used when `update_map.method` is 2. When `> 0`, a hit point that falls within this fraction of a local BHM's cell size from one of its faces triggers creating the neighboring BHM across that face, so a surface lying near the cell boundary is not cut off. Expressed as a fraction of the cell size in $[0, 0.5]$ (e.g. `0.1` = 10%); `0` disables it. Only takes effect together with `include_neighbor_bhm`. |
| `max_update_dist` | Maximum distance from the sensor position for updating local BHMs. `-1` (or a large value) means no limit. |
...
| `var_scale` | Scale factor for the surface point's position/normal variance (used when `update_map.method` is 2). Because the variance is divided by $\lVert \nabla l(p) \rVert^2$ (which grows roughly as $1/\text{kernel_scale}^2$), a large value (e.g. $10^6$) is typical. |
| `var_max` | Maximum variance for the surface point's position/normal (used when `update_map.method` is 2). <br> $\text{var_position} = \text{var_normal} = \min\!\left(\text{var_scale} \times \dfrac{\left(l(p) - \text{surface_log_odds}\right)^2}{\lVert \nabla l(p) \rVert^2},\ \text{var_max}\right)$, where $\nabla l(p)$ is the BHM log-odds gradient at the surface point. |
...
new_gp_load_data_immediately: true  # if true, newly created GPs load their training data immediately
...
sdf_gp:
    ...
    edf_gp:
        ...
        use_exp_bias: true  # use a bias term in exp for numerical stability
...
- [3D Depth Camera, Cow And Lady](../config/cow_and_lady/bayesian_hilbert/sdf_mapping_float.yaml)
- [3D LiDAR, Newer College](../config/newer_college/bayesian_hilbert/sdf_mapping_float.yaml)
- [2D LiDAR, Gazebo Room](../config/gazebo/bayesian_hilbert/sdf_mapping_float.yaml)
...
| `test_query.max_test_valid_distance_var` | If the GP prediction has a variance smaller than this value, the prediction is considered good enough as the final prediction. Used when `use_smallest` is false. |
| **test_query.search_area_half_size** | Half size of the search area (in meters) to look for neighboring SDF GPs. If a query position cannot find a GP within the region, this value is automatically doubled (fallback search). If this value is too small, the fallback search will take too much time. |
...
| `min_num_gps_to_update` | Minimum number of GPs pending in an update. |
| `new_gp_load_data_immediately` | If true, a newly created SDF GP loads its training data immediately instead of deferring to the priority queue. |
...
| `sdf_gp.edf_gp.use_exp_bias` | If true, use a bias term inside the exponential of the log mapping for numerical stability. |
