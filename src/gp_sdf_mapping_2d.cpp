    ...
    GpSdfMapping2D::UpdateGps(double time_budget_us) {
        ...
        // add affected clusters
        {
            ...
            for (const auto &cluster_key: changed_clusters) {
                const geometry::Aabb2D area(quadtree->KeyToCoord(cluster_key, cluster_depth), area_half_size);
                for (auto it = quadtree->BeginTreeInAabb(area, cluster_depth), end = quadtree->EndTreeInAabb(); it != end; ++it) {
