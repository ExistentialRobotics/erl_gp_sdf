...
namespace erl::sdf_mapping {
    ...
    void
    GpSdfMapping2D::TestBuffer::DisconnectBuffers() {
        positions = nullptr;
        distances = nullptr;
        gradients = nullptr;
        variances = nullptr;
    ...
    GpSdfMapping2D::Test(
        ...
        double gp_test_time = 0;
        {
            ...
            if (m_setting_->use_occ_sign) {
                ...
                const auto tree = m_surface_mapping_->GetQuadtree();
...
#pragma omp parallel for default(none) shared(tree, positions_in, num_queries)
                for (uint32_t i = 0; i < num_queries; i++) {
                    ...
                    const auto node = tree->Search(position.x(), position.y());
    ...
    bool
    GpSdfMapping2D::operator==(const GpSdfMapping2D &other) const {
    ...
    GpSdfMapping2D::Read(std::istream &s) {
        ...
        ERL_WARN("Failed to read GpSdfMapping2D. Truncated file?");
        return false;  // should not reach here
    }

    void
    GpSdfMapping2D::UpdateGps(double time_budget_us) {
        ERL_BLOCK_TIMER();
        ...
        const std::shared_ptr<SurfaceMappingQuadtree> tree = m_surface_mapping_->GetQuadtree();
        ...
        // add affected clusters
        {
            ...
            for (const auto &cluster_key: changed_clusters) {
                const geometry::Aabb2D area(tree->KeyToCoord(cluster_key, cluster_depth), area_half_size);
    ...
    void
    GpSdfMapping2D::TrainGpThread(const uint32_t thread_idx, const std::size_t start_idx, const std::size_t end_idx) {
        ...
        for (uint32_t i = start_idx; i < end_idx; ++i) {
            ...
            const geometry::Aabb2D area(tree->KeyToCoord(cluster_key, cluster_depth), aabb_half_size);
    ...
    void
    GpSdfMapping2D::SearchCandidateGps(const Eigen::Ref<const Eigen::Matrix2Xd> &positions_in) {
        ERL_BLOCK_TIMER();
        // collect some numbers needed for searching
        const double search_area_half_size = m_setting_->test_query->search_area_half_size;
        const auto tree = m_surface_mapping_->GetQuadtree();
        const uint32_t cluster_depth = tree->GetTreeDepth() - m_surface_mapping_->GetClusterLevel();
        const geometry::Aabb2D tree_aabb = tree->GetMetricAabb();  // biggest AABB of the tree
        geometry::Aabb2D query_aabb(                               // current queried search area
            positions_in.rowwise().minCoeff().array() - search_area_half_size,
            positions_in.rowwise().maxCoeff().array() + search_area_half_size);
        geometry::Aabb2D region = query_aabb.Intersection(tree_aabb);  // current region of interest
        ...
        while (region.sizes().prod() > 0) {                            // search until the intersection is empty
            ...
            query_aabb = geometry::Aabb2D(query_aabb.center, 2.0 * query_aabb.half_sizes);
            geometry::Aabb2D new_region = query_aabb.Intersection(tree_aabb);
        ...
        if (!m_candidate_gps_.empty()) {
            Eigen::Matrix2Xd positions(2, m_candidate_gps_.size());
            for (std::size_t i = 0; i < m_candidate_gps_.size(); ++i) { positions.col(static_cast<long>(i)) = m_candidate_gps_[i].second->position; }
            m_kd_tree_candidate_gps_ = std::make_shared<geometry::KdTree2d>(std::move(positions));
        }
    ...
    GpSdfMapping2D::SearchGpThread(uint32_t thread_idx, std::size_t start_idx, std::size_t end_idx) {
        ...
        const uint32_t cluster_level = m_surface_mapping_->GetClusterLevel();
        const uint32_t cluster_depth = tree->GetTreeDepth() - cluster_level;
        const geometry::Aabb2D tree_aabb = tree->GetMetricAabb();

        for (uint32_t i = start_idx; i < end_idx; ++i) {
            const Eigen::Vector2d test_position = m_test_buffer_.positions->col(i);
            ...
            geometry::Aabb2D search_aabb(test_position, search_area_half_size);
            geometry::Aabb2D region = tree_aabb.Intersection(search_aabb);
            ...
            if (gps.empty()) {               // no gp found
                ...
                search_aabb = geometry::Aabb2D(test_position, search_area_half_size);
                geometry::Aabb2D new_region = tree_aabb.Intersection(search_aabb);
    ...
    void
    GpSdfMapping2D::TestGpThread(const uint32_t thread_idx, const std::size_t start_idx, const std::size_t end_idx) {
        ERL_TRACY_SET_THREAD_NAME(fmt::format("{}:{}", __PRETTY_FUNCTION__, thread_idx).c_str());
        (void) thread_idx;
                ...
                if (variances(0, tested_idx[0].first) < max_test_valid_distance_var) {
                    ...
                    if (compute_covariance) {
                        m_test_buffer_.covariances->col(i) <<
                            (covariance(0, j1) * w2 + covariance(0, j2) * w1) / w12,
                            (covariance(1, j1) * w2 + covariance(1, j2) * w1) / w12,
                            (covariance(2, j1) * w2 + covariance(2, j2) * w1) / w12;
                    }
                    // clang-format on
                    m_query_used_gps_[i][0] = gps[tested_idx[0].second].second.second;
                    m_query_used_gps_[i][1] = gps[tested_idx[1].second].second.second;
                }
            } else {
