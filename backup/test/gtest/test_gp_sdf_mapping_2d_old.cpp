...
static std::string g_window_name = "ERL_GP_SDF";
...
using SurfaceData = erl::gp_sdf::SurfaceDataManager<2>::SurfaceData;
using Gp = erl::gp_sdf::SdfGaussianProcess<2, SurfaceData>;
...
DrawGp(
    ...
    const Eigen::Matrix2Xd used_surface_points =
        gp1->edf_gp->GetTrainInputSamplesBuffer().block(0, 0, 2, gp1->edf_gp->GetNumTrainSamples());
    Eigen::Matrix2Xi used_surface_points_px =
        grid_map_info->MeterToPixelForPoints(used_surface_points);
...
struct OpenCvUserData {
    ...
    erl::gp_sdf::GpSdfMapping2D *sdf_mapping = nullptr;
    cv::Mat img;
};
...
OpenCvMouseCallback(const int event, const int x, const int y, int /*flags*/, void *userdata) {
    if (event == cv::EVENT_LBUTTONDOWN) {
        ...
        Eigen::Vector2d position =
            data->grid_map_info->PixelToMeterForPoints(Eigen::Vector2i(x, y));
        ...
        if (data->sdf_mapping->Test(position, distance, gradient, variances, covariances)) {
            ...
            // draw sdf
            auto radius =
                static_cast<int>(std::abs(distance[0]) / data->grid_map_info->Resolution(0));
            ...
            cv::circle(
                ...
                cv::Point2i(x, y),
                radius,
                cv::Scalar(0, 255, 0, 25),
                cv::FILLED);
            cv::add(img * 0.5, circle_layer * 0.5, img, circle_mask);

            // draw sdf variance
            radius = static_cast<int>(
                (std::sqrt(variances(0, 0)) + std::abs(distance[0])) /
                data->grid_map_info->Resolution(0));
            cv::circle(img, cv::Point2i(x, y), radius, cv::Scalar(0, 0, 255, 25), 1);

            // draw sdf gradient
            Eigen::VectorXi grad_pixel =
                data->grid_map_info->MeterToPixelForVectors(gradient.col(0));
            cv::arrowedLine(
                img,
                cv::Point(x, y),
...
TEST(GpSdfMapping2D, Build_Save_Load) {
    ...
    ASSERT_TRUE(
        g_options.use_gazebo_room_2d || g_options.use_house_expo_lidar_2d ||
        g_options.use_ucsd_fah_2d)
        << "Please specify one of --use-gazebo-room-2d, --use-house-expo-lidar-2d, "
           "--use-ucsd-fah-2d.";
    if (g_options.use_gazebo_room_2d) {
        ASSERT_TRUE(std::filesystem::exists(g_options.gazebo_train_file))
            << "Gazebo train data file " << g_options.gazebo_train_file << " does not exist.";
    }
    if (g_options.use_house_expo_lidar_2d) {
        ASSERT_TRUE(std::filesystem::exists(g_options.house_expo_map_file))
            << "HouseExpo map file " << g_options.house_expo_map_file << " does not exist.";
    ...
    if (g_options.use_ucsd_fah_2d) {
        ASSERT_TRUE(std::filesystem::exists(g_options.ucsd_fah_2d_file))
            << "ROS bag dat file " << g_options.ucsd_fah_2d_file << " does not exist.";
    }
    ...
    auto sdf_mapping_setting = std::make_shared<erl::gp_sdf::GpSdfMapping2D::Setting>();
    ERL_ASSERTM(
    ...
    if (g_options.use_gazebo_room_2d) {
        ...
        // prepare buffer
        max_update_cnt =
            static_cast<long>(train_data_loader.size() - g_options.init_frame) / g_options.stride +
            1;
        ...
        int cnt = 0;
        for (int i = g_options.init_frame; i < static_cast<int>(train_data_loader.size());
             i += g_options.stride, ++cnt) {
        ...
        erl::geometry::HouseExpoMapLidar2D house_expo_map(
            g_options.house_expo_map_file,
            g_options.house_expo_traj_file,
            0.2,
            lidar_setting,
            true,
            0.01);
        ...
        const auto bar = erl::common::ProgressBar::Open(bar_setting, std::cout);
        int cnt = 0;
        for (long i = g_options.init_frame; i < house_expo_map.Size();
             i += g_options.stride, ++cnt) {
        ...
        std::cerr << "Please specify one of --use-gazebo-room-2d, --use-house-expo-lidar-2d, "
                     "--use-ucsd-fah-2d."
                  << std::endl;
        return;
    }
    ...
    auto surface_mapping_setting =
        std::dynamic_pointer_cast<erl::gp_sdf::GpOccSurfaceMapping2D::Setting>(
            sdf_mapping_setting->surface_mapping);
    ...
    erl::gp_sdf::GpSdfMapping2D sdf_mapping(sdf_mapping_setting);
    auto surface_mapping = std::dynamic_pointer_cast<erl::gp_sdf::GpOccSurfaceMapping2D>(
        sdf_mapping.GetSurfaceMapping());

    using OccupancyQuadtreeDrawer =
        erl::geometry::OccupancyQuadtreeDrawer<erl::gp_sdf::SurfaceMappingQuadtree>;
    ...
    drawer->SetDrawTreeCallback([&](const OccupancyQuadtreeDrawer *self,
                                    cv::Mat &img,
                                    erl::gp_sdf::SurfaceMappingQuadtree::TreeIterator &it) {
        const uint32_t cluster_depth =
            surface_mapping->GetQuadtree()->GetTreeDepth() - surface_mapping->GetClusterLevel();
        const auto grid_map_info = self->GetGridMapInfo();
        if (it->GetDepth() == cluster_depth) {
            Eigen::Vector2i position_px =
                grid_map_info->MeterToPixelForPoints(Eigen::Vector2d(it.GetX(), it.GetY()));
        ...
        Eigen::Vector2i position_px = grid_map_info->MeterToPixelForPoints(
            surface_data_manager[it->surface_data_index].position);
        cv::Point position_px_cv(position_px[0], position_px[1]);
        cv::circle(
            img,
            cv::Point(position_px[0], position_px[1]),
            1,
            cv::Scalar(0, 0, 255, 255),
            -1);  // draw surface point
        Eigen::Vector2i normal_px = grid_map_info->MeterToPixelForVectors(
            surface_data_manager[it->surface_data_index].normal * g_options.surf_normal_scale);
    ...
    drawer->SetDrawLeafCallback([&](const OccupancyQuadtreeDrawer *self,
                                    cv::Mat &img,
                                    erl::gp_sdf::SurfaceMappingQuadtree::LeafIterator &it) {
        if (!it->HasSurfaceData()) { return; }
        const auto grid_map_info = self->GetGridMapInfo();
        Eigen::Vector2i position_px = grid_map_info->MeterToPixelForPoints(
            surface_data_manager[it->surface_data_index].position);
    ...
    if (g_options.visualize) {
        pangolin_plotter_window =
            std::make_shared<erl::common::PangolinWindow>(g_window_name + ": curves", 1280, 960);
        ...
        pangolin_map_window = std::make_shared<erl::common::PangolinWindow>(
            g_window_name + ": map",
            img.cols,
            img.rows);
        pangolin_plotter_map = std::make_shared<erl::common::PangolinPlotterImage>(
            pangolin_map_window,
            img.rows,
            img.cols,
            GL_RGBA,
    ...
    std::string video_path =
        (std::filesystem::path(g_options.output_dir) / "sdf_mapping.avi").string();
    cv::Mat video_frame;
    ...
    for (long i = 0; i < max_update_cnt; i++) {
        ...
        if (g_options.visualize) {
            ...
            // draw trajectory
            erl::common::DrawTrajectoryInplace(
                img,
                cur_traj.leftCols(i),
                grid_map_info,
                trajectory_color,
            ...
            if (pangolin_map_window != nullptr && pangolin_plotter_map != nullptr) {
                ...
                plotter_sdf->Append(
                    ...
                    {static_cast<float>(distance[0]),
                     static_cast<float>(std::abs(distance[0])),
                     static_cast<float>(variances(0, 0))});
                plotter_grad->Append(
                    static_cast<float>(t),
                ...
                if (g_options.save_video) {
                    pangolin::TypedImage buffer = pangolin::ReadFramebuffer(
                        pangolin_plotter_window->GetDisplay("main").v,
                        "BGR24");
                    cv::Mat tmp(
                        static_cast<int>(buffer.h),
                        static_cast<int>(buffer.w),
                        CV_8UC3,
                        buffer.ptr);
                    cv::flip(tmp, tmp, 0);
        ...
        if (g_options.test_io && (i == 0 || i == max_update_cnt - 1)) {  // test io
            ...
            erl::gp_sdf::GpSdfMapping2D sdf_mapping_load(
                std::make_shared<erl::gp_sdf::GpSdfMapping2D::Setting>());
            ASSERT_TRUE(sdf_mapping_load.Read(bin_file)) << "Failed to read from " << bin_file;
            ASSERT_TRUE(sdf_mapping == sdf_mapping_load)
    ...
    if (success && g_options.visualize) {
        ...
        Eigen::MatrixX8U distances_out_mat_normalized =
        ...
        pangolin_sdf_image_window = std::make_shared<erl::common::PangolinWindow>(
            g_window_name + ": sdf",
            dst.cols,
            dst.rows);
        pangolin_plotter_sdf_image = std::make_shared<erl::common::PangolinPlotterImage>(
            pangolin_sdf_image_window,
            dst.rows,
            dst.cols,
            GL_RGBA,
            GL_UNSIGNED_BYTE);
        ...
        pangolin_sdf_sign_image_window = std::make_shared<erl::common::PangolinWindow>(
            g_window_name + ": sdf_sign",
            src.cols,
            src.rows);
        pangolin_plotter_sdf_sign_image = std::make_shared<erl::common::PangolinPlotterImage>(
            pangolin_sdf_sign_image_window,
            src.rows,
            src.cols,
            GL_RGBA,
            GL_UNSIGNED_BYTE);
        ...
        Eigen::MatrixXd sdf_variances_mat =
        ...
        Eigen::MatrixX8U variances_out_mat_normalized =
        ...
        pangolin_sdf_var_image_window = std::make_shared<erl::common::PangolinWindow>(
            g_window_name + ": sdf_variances",
            dst.cols,
            dst.rows);
        pangolin_plotter_sdf_var_image = std::make_shared<erl::common::PangolinPlotterImage>(
            pangolin_sdf_var_image_window,
            dst.rows,
            dst.cols,
            GL_RGBA,
            GL_UNSIGNED_BYTE);
    ...
    auto process_event = [&]() {
        if (pangolin_plotter_window != nullptr) {
            pangolin_plotter_window->GetWindow().ProcessEvents();
        }
        ...
        if (pangolin_sdf_sign_image_window != nullptr) {
            pangolin_plotter_sdf_sign_image->Render();
        }
