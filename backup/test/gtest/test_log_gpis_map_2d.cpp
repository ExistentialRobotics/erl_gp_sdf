...
TEST(ERL_GP_SDF, LogGpisMap2D) {
    ...
    auto train_data_loader =
        erl::geometry::GazeboRoom2D::TrainDataLoader((dir_path / train_dat_file).c_str());
    for (auto df: train_data_loader) {
        ss.str(std::string());
        ss << "LogGpisMap2D-Train[" << cnt << ']';
        t_train_ans += ReportTime<std::chrono::microseconds>(ss.str().c_str(), 0, true, [&]() {
            log_gpis_map.Update(df.angles, df.distances, df.pose_numpy);
        });
        cnt++;
        std::cout << "===============================================" << std::endl;
    }
    ...
    double t_test_ans =
        ReportTime<std::chrono::milliseconds>("LogGpisMap2D-test", 10, false, [&]() {
            log_gpis_map.Test(
                df.positions,
                distance_ans,
                gradient_ans,
                distance_variance_ans,
                gradient_variance_ans);
        });
    ...
    std::cout << "LogGpisMap2D: " << t_test_ans << " ms for " << df.positions.cols() << " points, "
              << t_per_point << " us per point" << std::endl;
}
